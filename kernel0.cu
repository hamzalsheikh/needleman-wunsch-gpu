...
__global__ void nw_upper_left_kernel0(unsigned char* reference, unsigned char* query, int* matrix, unsigned int N, int verticalBlockOffset, int horizontalBlockOffset) {
...
__global__ void nw_lower_right_kernel0(unsigned char* reference, unsigned char* query, int* matrix, unsigned int N, int verticalBlockOffset, int horizontalBlockOffset) {
...
void nw_gpu0(unsigned char* reference_d, unsigned char* query_d, int* matrix_d, unsigned int N) {
    ...
    for (int i = 0; i < numKernelCalls; i++) {
        ...
        nw_upper_left_kernel0<<< numBlocks, numThreadsPerBlock >>>(reference_d, query_d, matrix_d, N, i, 0);
		
        cudaDeviceSynchronize();

        // Second call concerns the lower right triangles
        nw_lower_right_kernel0<<< numBlocks, numThreadsPerBlock >>>(reference_d, query_d, matrix_d, N, i, 0);
    ...
    for (int i = numKernelCalls - 2; i >= 0; i--) {
        ...
        nw_upper_left_kernel0<<< numBlocks, numThreadsPerBlock >>>(reference_d, query_d, matrix_d, N, numKernelCalls - 1, horizontalOffset);
		
        cudaDeviceSynchronize();

        // Second call concerns the lower right triangles 
        nw_lower_right_kernel0<<< numBlocks, numThreadsPerBlock >>>(reference_d, query_d, matrix_d, N, numKernelCalls - 1, horizontalOffset);
